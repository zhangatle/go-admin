...
func (e *SysRole) Get() (SysRole SysRole, err error) {
...
func (e *SysRole) GetList() (SysRole []SysRole, err error) {
	table := orm.Eloquent.Table("sys_role")
	if e.RoleId != 0 {
		table = table.Where("role_id = ?", e.RoleId)
	}
...
// 获取角色对应的菜单ids
func (e *SysRole) GetRoleMeunId() ([]int, error) {
	menuIds := make([]int, 0)
	menuList := make([]MenuIdList, 0)
	if err := orm.Eloquent.Table("sys_role_menu").Select("sys_role_menu.menu_id").Joins("LEFT JOIN sys_menu on sys_menu.menu_id=sys_role_menu.menu_id").Where("role_id = ? ", e.RoleId).Where(" sys_role_menu.menu_id not in(select sys_menu.parent_id from sys_role_menu LEFT JOIN sys_menu on sys_menu.menu_id=sys_role_menu.menu_id where role_id =? )", e.RoleId).Find(&menuList).Error; err != nil {
		return nil, err
	}
...
func (e *SysRole) Insert() (id int, err error) {
	i := 0
	orm.Eloquent.Table("sys_role").Where("role_name=? or role_key = ?", e.RoleName, e.RoleKey).Count(&i)
	if i > 0 {
		return 0, errors.New("角色名称或者角色标识已经存在！")
	}
	e.UpdateBy = ""
	result := orm.Eloquent.Table("sys_role").Create(&e)
	if result.Error != nil {
		err = result.Error
		return
	}
...
func (e *SysRole) GetRoleDeptId() ([]int, error) {
	deptIds := make([]int, 0)
	deptList := make([]DeptIdList, 0)
	if err := orm.Eloquent.Table("sys_role_dept").Select("sys_role_dept.dept_id").Joins("LEFT JOIN sys_dept on sys_dept.dept_id=sys_role_dept.dept_id").Where("role_id = ? ", e.RoleId).Where(" sys_role_dept.dept_id not in(select sys_dept.parent_id from sys_role_dept LEFT JOIN sys_dept on sys_dept.dept_id=sys_role_dept.dept_id where role_id =? )", e.RoleId).Find(&deptList).Error; err != nil {
		return nil, err
	}
...
//修改
func (e *SysRole) Update(id int) (update SysRole, err error) {
	if err = orm.Eloquent.Table("sys_role").First(&update, id).Error; err != nil {
		return
	}

	if e.RoleName != "" && e.RoleName != update.RoleName {
		return update, errors.New("角色名称不允许修改！")
	}

	if e.RoleKey != "" && e.RoleKey != update.RoleKey {
		return update, errors.New("角色标识不允许修改！")
	}

	//参数1:是要修改的数据
	//参数2:是修改的数据
	if err = orm.Eloquent.Table("sys_role").Model(&update).Updates(&e).Error; err != nil {
		return
	}
	return
}
